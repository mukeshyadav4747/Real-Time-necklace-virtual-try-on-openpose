"""
Real-time necklace renderer with ZERO lag.
No position/size smoothing for instant response.
"""
...
class NecklaceRenderer:
    """Real-time renderer with instant response."""
    ...
    def __init__(self, necklace_image_path: str):
        ...
        if self.necklace_image is None:
            raise ValueError(f"Could not load: {necklace_image_path}")
        
        # Ensure RGBA
        if self.necklace_image.shape[2] == 3:
            alpha = np.ones((self.necklace_image.shape[0], self.necklace_image.shape[1], 1),
                          dtype=self.necklace_image.dtype) * 255
            self.necklace_image = np.concatenate([self.necklace_image, alpha], axis=2)
        ...
        print(f"✅ Real-Time Necklace Renderer loaded (Zero Lag)")
    ...
    def render_necklace(self, frame: np.ndarray, neck_data: Dict) -> np.ndarray:
        """
        Render necklace with ZERO lag - instant position update.
        """
        ...
        # Final size (NO SMOOTHING - instant)
        ...
        # Calculate position (NO SMOOTHING - instant)
    ...
    def _overlay_with_alpha(self, background, overlay, x, y):
        """Alpha blend."""
